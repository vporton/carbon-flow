...
async function mainTest(chai, deployments, numberOfCredits) {
    ...
    const createCreditEventAbi = JSON.parse(fs.readFileSync('artifacts/Carbon.json')).abi;
    ...
    for(let i = 0; i < authoritiesData.length; ++i) {
      ...
      const tx4 = await communityFundDAO.invoke(carbon, '0', 'setNonRecurringFlow', token, veryBigAmount);
    ...
    for(let netReducer of smartWallets) { // TODO: more tests
      for(let token of authorityTokens) {
        ...
        if(!balance.eq(ethers.BigNumber.from(0))) {
          const tx = await netReducer.invoke(carbon, '0', 'exchangeToParent', token, balance, ethers.BigNumber.from(1), []);
          await ethers.provider.getTransactionReceipt(tx.hash);
        }
      }
    }
