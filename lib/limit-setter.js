...
class LimitSetter {
    constructor(contract, smartWallet) {
        ...
        console.log('constructor: ', new ethers.BigNumber.from(this.hash).toString());
    }
    async setNonRecurringFlow(token, remaining) {
        ...
        const tx = await this.smartWallet.invoke(this.contract, '0', 'setNonRecurringFlow', token, remaining, this.hash);
        this.hash = this.calculate_hash(data);
        return tx;
    }
    async setRecurringFlow(token, maxSwapCredit, remainingSwapCredit, swapCreditPeriod, timeEnteredSwapCredit) {
        ...
        const tx = await this.smartWallet.invoke(this.contract, '0', 'setRecurringFlow', token, maxSwapCredit, remainingSwapCredit, swapCreditPeriod, timeEnteredSwapCredit, this.hash);
        this.hash = this.calculate_hash(data);
        return tx;
    }
    calculate_hash(data) {
